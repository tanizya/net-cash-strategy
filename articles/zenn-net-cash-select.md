...
最初はTradingView MCPを試してみたかっただけだった。「Claude CodeからPine Scriptをコンパイルしてバックテスト回せるの面白いな」程度のノリで触り始めたのに、気づいたらスクリーニングロジックを5回書き直し、NI225フィルターで4回失敗し、Supabaseにデータベースを立て、GitHub Actionsで毎日自動更新するダッシュボードを作り込んでいた。完全に沼。
